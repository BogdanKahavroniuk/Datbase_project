...
class Database():
    ...
    def  load_data( self    ) :
       ...
       file_links   =  [f"Sources/OData{year}File.csv"
                        ...
                        else   f"Sources/OpenData{year}.csv"
                        for   year in   years]
       ...
       for  year  in   years:
          ...
          if year in [2017 , 2018 , 2021 , 2022]:

              df = pd.read_csv( year_links[year] , delimiter = ';'
              ...
              df["year"] =  [year]*df.shape[0]
                  ...
                  df = pd.read_csv(year_links[year], delimiter=';'
                                   , encoding='ANSI', low_memory=False)
                  df.columns = df.columns.str.lower()
                  df["year"] = [year] * df.shape[0]
